...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    //usata
    @Query(value = "select * from reservations where user = :userId and start_date >= :sDate and end_date <= :eDate", nativeQuery = true)
    List<Reservation> findReservationPerUserAndPerDay(Date sDate, Date eDate, Long userId);

    //usata
    @Query(value = "select * from reservations where user = :userId", nativeQuery = true)
    List<Reservation> findReservationPerUser(Long userId);

    @Query(value = "select * from reservations where user = :userId and field = :fieldId", nativeQuery = true)
    List<Reservation> findReservationPerUserAndPerField(Long userId, Long fieldId);

    @Query(value = "select * from reservations where user = :userId and field = :fieldId and start_date >= :sDate and end_date <= :eDate", nativeQuery = true)
    List<Reservation> findReservationPerUserAndPerFieldAndDay(Date sDate, Date eDate, Long userId, Long fieldId);

    @Query(value = "select * from reservations where start_date >= :sDate and end_date <= :eDate", nativeQuery = true)
    List<Reservation> findReservationPerDate(Date sDate, Date eDate);

    @Query(value = "select * from reservations where field = :fieldId", nativeQuery = true)
